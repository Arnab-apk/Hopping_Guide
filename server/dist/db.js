...
exports.generateSquadCode = generateSquadCode;
exports.upsertProfile = upsertProfile;
exports.createSquad = createSquad;
exports.joinSquad = joinSquad;
exports.getSquadById = getSquadById;
exports.getSquadByCode = getSquadByCode;
exports.getSquadMembers = getSquadMembers;
exports.leaveSquad = leaveSquad;
exports.updateMeetup = updateMeetup;
exports.updateSettings = updateSettings;
exports.upsertLocation = upsertLocation;
exports.getLatestLocations = getLatestLocations;
...
class InMemoryNeonStore {
    profiles = new Map();
    squads = new Map(); // id -> SquadRow
    squadCodeMap = new Map(); // code -> id
    members = new Map(); // squad_id -> (user_id -> MemberRow)
    locations = new Map(); // squad_id -> (user_id -> LocationRow)
    messages = new Map(); // squad_id -> MessageRow[]
    events = new Map(); // squad_id -> EventRow[]
}
const memStore = new InMemoryNeonStore();
// ------------------------------------------------------------------------------
// Database Operations Facade
// ------------------------------------------------------------------------------
async function upsertProfile(userId, displayName, avatarUrl, isGuest = true) {
    const now = new Date().toISOString();
    if (pool) {
        const res = await pool.query(`INSERT INTO profiles (user_id, display_name, avatar_url, is_guest, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $5)
       ...
       RETURNING *;`, [userId, displayName, avatarUrl || null, isGuest, now]);
        return res.rows[0];
    }
...
async function createSquad(params) {
    ...
    if (pool) {
        ...
        try {
            ...
            if (params.hostDisplayName) {
                await client.query(`INSERT INTO profiles (user_id, display_name, avatar_url, is_guest, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $5)
           ON CONFLICT (user_id) DO UPDATE SET display_name = EXCLUDED.display_name, updated_at = EXCLUDED.updated_at;`, [params.hostUserId, params.hostDisplayName, params.hostAvatarUrl || null, params.isGuest ?? true, now]);
            }
            // 2. Insert squad
            const squadRes = await client.query(`INSERT INTO squads (id, code, name, host_user_id, meetup_lat, meetup_lng, meetup_label, separation_radius_m, status, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'active', $9, $9)
         RETURNING *;`, [squadId, code, params.name, params.hostUserId, params.meetupLat ?? null, params.meetupLng ?? null, params.meetupLabel ?? null, radius, now]);
            // 3. Insert host into squad_members
            const memberRes = await client.query(`INSERT INTO squad_members (squad_id, user_id, role, share_location, joined_at, last_seen_at)
         VALUES ($1, $2, 'host', true, $3, $3)
         RETURNING *;`, [squadId, params.hostUserId, now]);
            // 4. Record audit event
            await client.query(`INSERT INTO squad_events (id, squad_id, actor_user_id, event_type, payload, created_at)
         VALUES ($1, $2, $3, 'squad_created', $4, $5);`, [(0, uuid_1.v4)(), squadId, params.hostUserId, JSON.stringify({ code, name: params.name }), now]);
    ...
    await upsertProfile(params.hostUserId, params.hostDisplayName || 'Hopper Host', params.hostAvatarUrl, params.isGuest ?? true);
    const squad = {
        id: squadId,
        code,
        name: params.name,
...
async function joinSquad(params) {
    ...
    if (pool) {
        ...
        try {
            await client.query('BEGIN');
            // 1. Find squad
            const squadRes = await client.query(`SELECT * FROM squads WHERE code = $1;`, [normCode]);
            ...
            if (params.displayName) {
                await client.query(`INSERT INTO profiles (user_id, display_name, avatar_url, is_guest, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $5)
           ON CONFLICT (user_id) DO UPDATE SET display_name = EXCLUDED.display_name, updated_at = EXCLUDED.updated_at;`, [params.userId, params.displayName, params.avatarUrl || null, params.isGuest ?? true, now]);
            }
            // 3. Check existing membership (Idempotent)
            const existingRes = await client.query(`SELECT * FROM squad_members WHERE squad_id = $1 AND user_id = $2;`, [squad.id, params.userId]);
            let member;
            let isNewJoin = false;
            if (existingRes.rows.length > 0) {
                // Update last seen
                const updateRes = await client.query(`UPDATE squad_members SET last_seen_at = $1 WHERE squad_id = $2 AND user_id = $3 RETURNING *;`, [now, squad.id, params.userId]);
                member = updateRes.rows[0];
            }
            else {
                // Insert new member
                const insertRes = await client.query(`INSERT INTO squad_members (squad_id, user_id, role, share_location, joined_at, last_seen_at)
           VALUES ($1, $2, 'member', true, $3, $3)
           RETURNING *;`, [squad.id, params.userId, now]);
                member = insertRes.rows[0];
                isNewJoin = true;
                // Record join event
                await client.query(`INSERT INTO squad_events (id, squad_id, actor_user_id, event_type, payload, created_at)
           VALUES ($1, $2, $3, 'member_joined', $4, $5);`, [(0, uuid_1.v4)(), squad.id, params.userId, JSON.stringify({ displayName: params.displayName }), now]);
            }
            await client.query('COMMIT');
    ...
    await upsertProfile(params.userId, params.displayName || 'Companion Hopper', params.avatarUrl, params.isGuest ?? true);
...
async function getSquadByCode(code) {
...
async function getSquadMembers(squadId) {
    if (pool) {
        const res = await pool.query(`SELECT sm.*, p.display_name, p.avatar_url, p.is_guest
       FROM squad_members sm
       LEFT JOIN profiles p ON sm.user_id = p.user_id
       WHERE sm.squad_id = $1
       ORDER BY (CASE WHEN sm.role = 'host' THEN 0 ELSE 1 END), sm.joined_at ASC;`, [squadId]);
        return res.rows;
    }
    ...
    if (pool) {
        ...
        try {
            ...
            if (squad.host_user_id === userId) {
                ...
                const membersRes = await client.query(`SELECT * FROM squad_members WHERE squad_id = $1 AND user_id != $2 ORDER BY joined_at ASC LIMIT 1;`, [squadId, userId]);
...
async function updateMeetup(squadId, hostUserId, meetup) {
    const now = new Date().toISOString();
    if (pool) {
        const res = await pool.query(`UPDATE squads
       SET meetup_lat = $1, meetup_lng = $2, meetup_label = $3, updated_at = $4
       WHERE id = $5 AND host_user_id = $6
       RETURNING *;`, [meetup.lat, meetup.lng, meetup.label, now, squadId, hostUserId]);
        if (res.rows.length === 0)
            throw new Error('NOT_HOST_OR_SQUAD_NOT_FOUND');
        return res.rows[0];
    }
    const squad = memStore.squads.get(squadId);
    if (!squad || squad.host_user_id !== hostUserId)
        throw new Error('NOT_HOST_OR_SQUAD_NOT_FOUND');
...
async function updateSettings(squadId, hostUserId, settings) {
    const now = new Date().toISOString();
    ...
    if (pool) {
       ...
       RETURNING *;`, [radius ?? null, settings.name ?? null, now, squadId, hostUserId]);
        if (res.rows.length === 0)
            throw new Error('NOT_HOST_OR_SQUAD_NOT_FOUND');
        return res.rows[0];
    }
    ...
    if (!squad || squad.host_user_id !== hostUserId)
        throw new Error('NOT_HOST_OR_SQUAD_NOT_FOUND');
    if (radius)
        squad.separation_radius_m = radius;
    if (settings.name)
        squad.name = settings.name;
    squad.updated_at = now;
    return squad;
}
async function upsertLocation(squadId, userId, loc) {
    const now = new Date().toISOString();
    if (pool) {
        await pool.query(`INSERT INTO squad_locations (squad_id, user_id, latitude, longitude, accuracy_m, heading_deg, speed_mps, updated_at)
       ...
       SET latitude = EXCLUDED.latitude,
           ...
           updated_at = EXCLUDED.updated_at;`, [squadId, userId, loc.latitude, loc.longitude, loc.accuracyM ?? null, loc.headingDeg ?? null, loc.speedMps ?? null, now]);
        return;
    }
...
async function getLatestLocations(squadId) {
    if (pool) {
        const res = await pool.query(`SELECT sl.* FROM squad_locations sl
       JOIN squad_members sm ON sl.squad_id = sm.squad_id AND sl.user_id = sm.user_id
       WHERE sl.squad_id = $1 AND sm.share_location = TRUE;`, [squadId]);
        return res.rows;
    }
    const locMap = memStore.locations.get(squadId);
    if (!locMap)
...
async function saveMessage(params) {
    ...
    if (pool) {
        const res = await pool.query(`INSERT INTO squad_messages (id, squad_id, sender_user_id, message_type, message, media_url, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *;`, [id, params.squadId, params.senderUserId, msgType, params.message || null, params.mediaUrl || null, now]);
        return res.rows[0];
    }
    const row = {
        id,
...
async function getMessages(squadId, limit = 50) {
    if (pool) {
        const res = await pool.query(`SELECT * FROM squad_messages
       WHERE squad_id = $1
       ORDER BY created_at ASC
       LIMIT $2;`, [squadId, limit]);
        return res.rows;
    }
